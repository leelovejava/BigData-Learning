package com.leelovejava.bigdata.spark.sql.dataframe;

import com.leelovejava.bigdata.spark.sql.Person;
    ...
    public static void main(String[] args) {
        ...
        javaRDD.map(new Function<Row, Person>() {
            ...
            public Person call(Row row) throws Exception {
                Integer id = Integer.parseInt(row.getAs("id").toString());
                String name = row.getAs("name").toString();
                String gender = row.getAs("gender").toString();
                Integer age = Integer.parseInt(row.getAs("age").toString());
