...
object UpdateStateByKey {
  def main(args: Array[String]): Unit = {
    ...
    val lines: ReceiverInputDStream[String] = ssc.socketTextStream("node05", 9999)
    ...
    /**
      * updateStateByKey
      * 注意：1.需要设置checkpoint 保存状态
      * 默认状态在内存中也有一份，多久将内存中的状态保存到checkpoint中一次呢?
      * 1).batchInterval 如果小于10s ,那么就10s保存一次
      * 2).如果batchInterval 大于10s ,那么就batchInterval保存一次
      *        2.updateStateByKey可以统计自从SparkStreaming启动以来所有key的状态。
      *
      */
    ...
    val result: DStream[(String, Int)] = pairWords.updateStateByKey((currentValues: Seq[Int], preValue: Option[Int]) => {
      // var totalValues: Int = preValue.getOrElse(0)
      var totalValues = 0
      if (!preValue.isEmpty) {
